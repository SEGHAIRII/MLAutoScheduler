...
int main(int argc, char **argv)
{
  ...
  std::cerr << "best evaluation = " << RootEvel << std::endl;
  ...
  stage = fixedSize;
  MonteCarlo search_method = MonteCarlo(&context, functionName);
  ...
  while (stage > 0)
  {
    ...
    std::cerr << " CURRENT STAGE FOR PARALLIZATION : " << stage << std::endl;
    bestEval = search_method.runSearchMethod(root, evaluator, linalgOps, 300);
    std::cerr << "BEST NODE FOUND WITH EXECUTION : " << bestEval->getEvaluation() << std::endl;
    root = bestEval;
    stage -= 1;
    root->setCurrentStage(stage);
  ...
  std::ofstream outputFile("./benchmark_MonteCarloEval_" + functionName + ".json");
